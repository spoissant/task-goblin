CREATE TABLE `blocked_by` (
	...
	`blocked_branch_id` integer,
	`blocker_task_id` integer,
	`blocker_todo_id` integer,
	`blocker_branch_id` integer,
	FOREIGN KEY (`blocked_task_id`) REFERENCES `tasks`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`blocked_branch_id`) REFERENCES `branches`(`id`) ON UPDATE no action ON DELETE no action,
	...
	FOREIGN KEY (`blocker_branch_id`) REFERENCES `branches`(`id`) ON UPDATE no action ON DELETE no action
);
