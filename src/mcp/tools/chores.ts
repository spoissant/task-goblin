...
export function registerChoreTools(server: McpServer) {
  ...
  server.registerTool(
    ...
    {
      description:
        "List all actionable chores across tasks — one entry per chore+task pair, ordered by chore number (priority) then task priority. Chores 1-7 are post-implementation housekeeping (failed checks, unresolved comments, draft review, review requests, merge conflicts, staging deploy, dev QA). Chore 8 is in-progress work. Chore 9 is tasks not yet started.",
      inputSchema: {
        repository: z
          .string()
          .optional()
